...
import { Briefcase, ExternalLink, ChevronRight, Zap, Award } from 'lucide-react';
...
export default function Experience() {
  ...
  return (
    <section id="experience" ref={ref} className="relative py-28 overflow-hidden grid-bg">
      {/* Decorative lines */}
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="section-title text-4xl mb-3" style={{ color: 'var(--text-primary)' }}>
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="orbitron text-sm font-bold tracking-widest" style={{ color: 'var(--magenta)' }}>
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                <div>
                  ...
                  <p className="mono text-sm mt-1" style={{ color: 'var(--magenta)' }}>
                    @ {exp.company}
                  </p>
                </div>
              ...
              <div className="space-y-2 mb-4">
                {exp.highlights.map((h, j) => (
                  <div key={j} className="flex gap-3">
                    <ChevronRight size={14} className="flex-shrink-0 mt-1" style={{ color: 'var(--magenta)' }} />
                    <p className="text-sm" style={{ color: 'var(--text-muted)', lineHeight: 1.7 }}>{h}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {exp.tags.map((t) => (
                  <span key={t} className="tag-badge" style={{ borderColor: 'rgba(255,0,110,0.3)', color: 'rgba(255,0,110,0.7)' }}>
                    {t}
                  </span>
                ))}
              </div>
        ...
        <div>
          <div className="flex items-center gap-3 mb-8">
            ...
            <span className="orbitron text-sm font-bold tracking-widest" style={{ color: 'var(--cyan)' }}>
              ENGINEERED SYSTEMS
            </span>
              ...
              >
                ...
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs font-semibold" style={{ color: 'var(--text-primary)', letterSpacing: '0.5px' }}>
                      {p.subtitle}
                    </p>
                    <p className="mono text-xs mt-1" style={{ color: 'var(--text-muted)', fontSize: '10px' }}>
                      {p.stack}
                    </p>
                  </div>
                  {p.links.live && (
                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                  {p.metrics.map((m) => (
                    ...
                    >
                      <div className="orbitron font-bold text-sm" style={{ color: p.color }}>
                        {m.value}
                      </div>
                      <div className="mono" style={{ color: 'var(--text-muted)', fontSize: '8px', letterSpacing: '0.5px' }}>
                        {m.label}
                      </div>
                    </div>
                  ))}
                ...
                <div className="flex-1 space-y-2 mb-4">
                  {p.highlights.map((h, j) => (
                    <div key={j} className="flex gap-2">
                      <ChevronRight size={12} className="flex-shrink-0 mt-1" style={{ color: p.color }} />
                      <p style={{ color: 'var(--text-muted)', fontSize: '12px', lineHeight: 1.6 }}>{h}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-1 mt-auto">
                  {p.tags.map((t) => (
                    <span
                      key={t}
                      className="tag-badge"
                      style={{ borderColor: `${p.color}30`, color: `${p.color}80`, fontSize: '9px' }}
                    >
                      {t}
                    </span>
                  ))}
